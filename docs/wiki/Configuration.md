...
| `agent_data_cache` | permanent SQLite cache for routed agent tool data (`enabled` default true; `db_path` optional; `read_ttl_seconds` default 86400; `max_entries` default 50000; `skip_empty_results` default true) |
...
Beyond the keys in [Getting Started](Getting-Started.md): `MOSAIC_PYTHON` (interpreter), `MOSAIC_DATA_DIR` / `MOSAIC_RESULTS_DIR` / `MOSAIC_CACHE_DIR` (artefact roots), `MOSAIC_AGENT_DATA_CACHE_ENABLED` / `MOSAIC_AGENT_DATA_CACHE_DB` / `MOSAIC_AGENT_DATA_CACHE_READ_TTL_SECONDS` / `MOSAIC_AGENT_DATA_CACHE_MAX_ENTRIES` / `MOSAIC_AGENT_DATA_CACHE_SKIP_EMPTY_RESULTS` (permanent routed tool cache), `MOSAIC_BENCHMARK_TICKER` (scorer benchmark), `QLIB_CN_DATA_PATH` / `QLIB_CN_ETF_PATH` (qlib datasets), `MOSAIC_QLIB_REPO` / `MOSAIC_QLIB_ETF_COLLECTOR` (collector discovery), `MOSAIC_MIROFISH_URL` (OASIS engine).
